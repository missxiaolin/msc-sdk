  ...
  let MethodLibrary = (function () {
    return {
      ...
      getDevice() {
        ...
        // "Mozilla/5.0 (Linux; Android 6.0.1; Nexus 7 Build/MOB30X) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Safari/537.36"
        // "Mozilla/5.0 (iPhone; CPU iPhone OS 13_2_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.0.3 Mobile/15E148 Safari/604.1"
        // "Mozilla/5.0 (iPhone; CPU iPhone OS 13_2_3 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/13.0.3 Mobile/15E148 Safari/604.1"

        if (['Mobile', 'Tablet', 'iPad'].includes(_this.device)) {
          try {
            if (u.includes('Brand')) {
              device = /\Brand\/(.+?)\;/g.exec(u)[1];
            } else if (_this.os == 'Android') {
              device = /\; (.+?)\ Build/g.exec(u)[1];
              if (device.includes(';')) {
                const sp = device.split('; ');
                device = sp[sp.length - 1];
              }
            } else if (_this.os == 'iOS') {
              device = 'iPhone';
            }
          } catch (e) {
            device = _this.os;
      ...
      getBrowserInfo() {
        ...
        return `${_this.browser} ${_this.browserVersion} (${_this.engine})`; // _this.browser + '（版本: ' + _this.browserVersion + '&nbsp;&nbsp;内核: ' + _this.engine + '）'
      },
    };
  })();
  // 逻辑层
    ...
    return {
      DeviceInfoObj(params) {
        ...
        let info = {
          ...
          OS: `${MethodLibrary.getOS()} ${MethodLibrary.getOSVersion()}`, // 操作系统
          ...
          device: MethodLibrary.getDevice(), // 设备类型
