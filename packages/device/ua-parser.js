...
let NAVIGATOR = typeof window !== UNDEF_TYPE && window.navigator ? window.navigator : undefined,
...
var extend = function (regexes, extensions) {
    ...
    itemListToArray = function (header) {
        ...
        let arr = []
        let tokens = strip(/\\?\"/g, header).split(', ')
        for (let i = 0; i < tokens.length; i++) {
            let token = tokens[i].split(';v=')
    ...
    fingerPrint = function (domain) {
        let fingerprint = localStorage.getItem('webFingerprint') || ''
...
let defaultProps = (function () {
    let props = { init: {}, isIgnore: {}, isIgnoreRgx: {}, toString: {} }
...
let createIData = function (item, itemType) {
    let init_props = defaultProps.init[itemType],
    ...
    // nodejs / non-client-hints browsers
        ...
        return NAVIGATOR_UADATA.getHighEntropyValues(CH_ALL_VALUES).then(res => item.setCH(new UACHData(res, false)).parseCH().get())
    }
    ...
    if (!NAVIGATOR_UADATA) {
        IData.prototype.then = function (cb) {
            ...
            let resolveData = new IDataResolve()
            cb(resolveData)
            return resolveData
        }
    }
    ...
    this.parseCH = function () {
        ...
        switch (this.itemType) {
            ...
            case UA_OS:
                ...
                if (osName) {
                    let osVersion = uaCH[PLATFORMVER]
...
function UAParser(ua, extensions, headers) {
    ...
    let userAgent = typeof ua === STR_TYPE
