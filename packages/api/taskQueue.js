...
const TaskQueue = {
  ...
  init(options = {}) {
    ...
    // 初始化获取缓存信息
    TaskQueue.getStorageQueses();
  },
  /**
   * 获取缓存内 上报信息
   */
  getStorageQueses() {
    let storageList = window.localStorage.getItem('webMonitorQueues');
    if (storageList) {
      storageList = JSON.parse(storageList);
      this.queues = this.queues.concat(storageList);
    }
    window.localStorage.removeItem('webMonitorQueues');
  },
  /**
   * 添加消息
   * @param {*} $options 配置
  ...
  add(data) {
    ...
    // window.localStorage.setItem('webMonitorQueues')
  },
  /**
  ...
  windowHidden() {
    ...
    window.localStorage.setItem('webMonitorQueues', list);
  },
  ...
  sendEscalation(activelyReportData = []) {
    let deviceInfo = TaskQueue.deviceInfo; // 设备信息
    // 执行自定义字段获取，不做持久存储，防里面有 回调函数
    let customInfo = TaskQueue.getCustomInfo();
    const { maxQueues, reportUrl, beforeSend, reportType, uuId, monitorAppId, encryption } =
      this.apiOtion;
    // 兼容 uuid 动态获取... 修复页面第一获取用户信息失败 都用guid
